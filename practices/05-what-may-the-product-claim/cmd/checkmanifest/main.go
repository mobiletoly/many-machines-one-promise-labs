...
const manifestPath = "practices/05-what-may-the-product-claim/practice.json"
...
func validateManifest(candidate manifest) error {
	if candidate.Number != 5 || candidate.ID != "what-may-the-product-claim" || candidate.Type != "practice" ||
		candidate.Title != "What May the Product Claim?" || candidate.RecommendedAfterChapter != 24 {
		return fmt.Errorf("manifest identity does not match Practice 05")
	}
	wantCapabilities := []string{"multi-authority-outcome-review", "corrective-obligation-review", "product-claim-boundary", "product-contract-architecture-fit", "guarantee-boundary"}
	if !sameStrings(candidate.Capabilities, wantCapabilities) {
		return fmt.Errorf("manifest capabilities do not match Practice 05")
	}
	wantArtifacts := []string{"contract", "workflowDesign", "authorityEvidence", "claimCandidates", "starterReview", "solutionReview"}
	...
	for _, name := range wantArtifacts {
		...
		if !strings.HasPrefix(path, "practices/05-what-may-the-product-claim/") {
			return fmt.Errorf("manifest artifact %q leaves Practice 05", name)
		}
...
func fatalf(format string, arguments ...any) {
	fmt.Fprintf(os.Stderr, "Practice 05 manifest check: "+format+"\n", arguments...)
	os.Exit(1)
}
