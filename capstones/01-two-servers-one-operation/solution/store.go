...
func (s *Store) Create(ctx context.Context, operationID string, request CreateRequest) (Order, error) {
	...
	created, err := s.insert(ctx, operationID, request)
	if err == nil {
		return created, nil
	}
	if !isOperationIdentityConflict(err) {
		return Order{}, err
	}

	retained, err = s.find(ctx, operationID)
	if err != nil {
		return Order{}, fmt.Errorf("read canonical order after uniqueness conflict: %w", err)
	}
	return matchingResult(retained, request)
}
...
func isOperationIdentityConflict(err error) bool {
	var pgError *pgconn.PgError
	return errors.As(err, &pgError) &&
		pgError.Code == "23505" &&
		pgError.ConstraintName == "one_canonical_order_per_operation"
}
