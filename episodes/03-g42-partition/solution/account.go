...
func (a *Account) Merge(remote Snapshot) error {
	...
	incoming := make(map[string]Redemption, len(remote.Operations))
	for _, operation := range remote.Operations {
		if retained, ok := incoming[operation.OperationID]; ok && retained != operation {
			return ErrOperationConflict
		}
		incoming[operation.OperationID] = operation
	}
	for operationID, operation := range incoming {
		if retained, ok := a.operations[operationID]; ok && retained != operation {
			return ErrOperationConflict
		}
	}
	for operationID, operation := range incoming {
